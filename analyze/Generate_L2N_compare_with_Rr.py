...
for i in df['bp_parms'].unique():
    ...
    y_rmlfq = x['Rmlfq_L2_Norm/RR_L2_Norm'].tolist()
    plt.clf()
    plt.figure(figsize=(30,20))  # Adjust the figure size as needed
    plt.axhline(y=1, color='r', linestyle='--', label='Y=1')
    plt.plot(x_label, y_rmlfq,'-^',label="RMLFQ_L2_Norm/RR_L2_Norm")
    ...
    img_name = i.replace("/","_") + "_l2_norm_compare_Rr_result.jpg"
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, img_name), bbox_inches="tight")
    plt.close()

arithmetic_ls = ['Rmlfq_L2_Norm/RR_L2_Norm']
...
df_melt.to_csv("Rmlfq_l2norm_compare_with_Rr.csv",index=False)