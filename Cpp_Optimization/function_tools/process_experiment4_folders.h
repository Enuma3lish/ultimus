#ifndef PROCESS_EXPERIMENT4_FOLDERS_H
#define PROCESS_EXPERIMENT4_FOLDERS_H
 ...
 *
 * This experiment tests system behavior under overload conditions.
 * Job size parameters switch between BP and Normal from avg_30.
 * Inter-arrival time is fixed at 20, creating ρ = 30/20 = 1.5 (overload).
 */

template<typename AlgoFunc>
void process_experiment4_folders(AlgoFunc algo, const std::string& algo_name,
                                 const std::string& data_dir, const std::string& output_dir) {
    std::string exp4_result_dir = output_dir + "/experiment4_fixed_arrival_20_result";
    create_directory(exp4_result_dir);
    ...
    for (const auto& folder : folders) {
        ...
        if (basename.find("experiment4_fixed_arrival_20_") == std::string::npos ||
            !directory_exists(folder)) {
            continue;
        }

        int base_version = extract_version_from_path(basename);
        std::cout << "Processing experiment4_fixed_arrival_20 base: " << basename
        ...
        for (const auto& freq_folder : freq_folders) {
            ...
            for (const auto& file : files) {
                ...
                if (filename.find("exp4_fixed_arrival20_freq_") == std::string::npos ||
    ...
    for (auto& pair : results_by_version) {
        ...
        if (pair.first >= 0) {
            output_file = exp4_result_dir + "/experiment4_fixed_arrival_20_result_" +
                         algo_name + "_" + std::to_string(pair.first) + ".csv";
        } else {
            output_file = exp4_result_dir + "/experiment4_fixed_arrival_20_result_" + algo_name + ".csv";
        }
    ...
    std::cout << "\nExperiment 4 processing complete!" << std::endl;
}

#endif // PROCESS_EXPERIMENT4_FOLDERS_H
