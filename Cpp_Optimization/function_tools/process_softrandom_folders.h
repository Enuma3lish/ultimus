...
// Multimode function for Bounded Pareto softrandom - Dynamic
template<typename MultiModeFunc>
void process_bounded_pareto_softrandom_folders_multimode(MultiModeFunc multi_mode_algo,
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        if (basename.find("Bounded_Pareto_softrandom_") == std::string::npos || !directory_exists(folder)) continue;
        folder_threads.emplace_back([&, folder, basename]() {
            int base_version = extract_version_from_path(basename);
            safe_cout("Processing Bounded_Pareto_softrandom base: " + basename + "\n");
            auto freq_folders = list_directory(folder);
            for (const auto& freq_folder : freq_folders) {
                std::string freq_basename = freq_folder.substr(freq_folder.find_last_of('/') + 1);
                ...
                for (const auto& file : files) {
                    std::string filename = file.substr(file.find_last_of('/') + 1);
                    if (filename.find("Bounded_Pareto_softrandom_freq_") == std::string::npos || filename.find(".csv") == std::string::npos) continue;
                    auto jobs = read_jobs_from_csv(file);
                    if (jobs.empty()) continue;
    ...
    for (auto& pair : results_by_version) {
        std::string output_file = bp_softrandom_result_dir + "/Bounded_Pareto_softrandom_result_Dynamic_njobs" +
...
// Multimode function for Normal softrandom - Dynamic
template<typename MultiModeFunc>
void process_normal_softrandom_folders_multimode(MultiModeFunc multi_mode_algo,
                                     const std::string& data_dir,
                                     const std::string& output_dir,
                                     int nJobsPerRound,
                                     const std::vector<int>& modes_to_run,
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        if (basename.find("normal_softrandom_") == std::string::npos || !directory_exists(folder)) continue;
        folder_threads.emplace_back([&, folder, basename]() {
            int base_version = extract_version_from_path(basename);
            safe_cout("Processing normal_softrandom base: " + basename + "\n");
            auto freq_folders = list_directory(folder);
            for (const auto& freq_folder : freq_folders) {
                std::string freq_basename = freq_folder.substr(freq_folder.find_last_of('/') + 1);
                if (freq_basename.find("freq_") == std::string::npos || !directory_exists(freq_folder)) continue;
                ...
                for (const auto& file : files) {
                    std::string filename = file.substr(file.find_last_of('/') + 1);
                    if (filename.find("normal_softrandom_freq_") == std::string::npos || filename.find(".csv") == std::string::npos) continue;
                    auto jobs = read_jobs_from_csv(file);
                    if (jobs.empty()) continue;
                    auto result_pair = multi_mode_algo(jobs, nJobsPerRound, modes_to_run);
                    std::map<int, double> mode_results = result_pair.first;
    ...
    for (auto& thread : folder_threads) thread.join();
    for (auto& pair : results_by_version) {
        std::string output_file = normal_softrandom_result_dir + "/normal_softrandom_result_Dynamic_njobs" +
                                 std::to_string(nJobsPerRound) + "_" + std::to_string(pair.first) + ".csv";
        std::ofstream out(output_file);
        out << "frequency";
        for (int mode : modes_to_run) out << ",Dynamic_njobs" << nJobsPerRound << "_mode" << mode << "_L2_norm_flow_time";
...
// Multimode function for Bounded Pareto softrandom - Dynamic_BAL
template<typename MultiModeFunc>
void process_bounded_pareto_softrandom_folders_multimode_DBAL(MultiModeFunc multi_mode_algo,
                                     const std::string& data_dir,
                                     const std::string& output_dir,
                                     int nJobsPerRound,
                                     const std::vector<int>& modes_to_run,
    ...
    for (auto& thread : folder_threads) thread.join();
    for (auto& pair : results_by_version) {
        std::string output_file = bp_softrandom_result_dir + "/Bounded_Pareto_softrandom_result_Dynamic_BAL_njobs" +
                                 std::to_string(nJobsPerRound) + "_" + std::to_string(pair.first) + ".csv";
        std::ofstream out(output_file);
        out << "frequency";
        for (int mode : modes_to_run) out << ",Dynamic_BAL_njobs" << nJobsPerRound << "_mode" << mode << "_L2_norm_flow_time";
        for (int mode : modes_to_run) out << ",Dynamic_BAL_njobs" << nJobsPerRound << "_mode" << mode << "_maximum_flow_time";
        out << "\n";
        for (const auto& result : pair.second) {
            out << result.at("frequency");
            for (int mode : modes_to_run) out << "," << result.at("l2_mode_" + std::to_string(mode));
...
// Multimode function for Normal softrandom - Dynamic_BAL
template<typename MultiModeFunc>
void process_normal_softrandom_folders_multimode_DBAL(MultiModeFunc multi_mode_algo,
                                     const std::string& data_dir,
                                     const std::string& output_dir,
                                     int nJobsPerRound,
                                     const std::vector<int>& modes_to_run,
    ...
    for (auto& thread : folder_threads) thread.join();
    for (auto& pair : results_by_version) {
        std::string output_file = normal_softrandom_result_dir + "/normal_softrandom_result_Dynamic_BAL_njobs" +
                                 std::to_string(nJobsPerRound) + "_" + std::to_string(pair.first) + ".csv";
        std::ofstream out(output_file);
        out << "frequency";
        for (int mode : modes_to_run) out << ",Dynamic_BAL_njobs" << nJobsPerRound << "_mode" << mode << "_L2_norm_flow_time";
...
// Multimode function for Bounded Pareto softrandom - RFDynamic
template<typename MultiModeFunc>
void process_bounded_pareto_softrandom_folders_multimode_RF(MultiModeFunc multi_mode_algo,
                                     const std::string& data_dir,
                                     const std::string& output_dir,
                                     int nJobsPerRound,
                                     const std::vector<int>& modes_to_run,
    ...
    for (auto& thread : folder_threads) thread.join();
    for (auto& pair : results_by_version) {
        std::string output_file = bp_softrandom_result_dir + "/Bounded_Pareto_softrandom_result_RFDynamic_njobs" +
                                 std::to_string(nJobsPerRound) + "_" + std::to_string(pair.first) + ".csv";
        std::ofstream out(output_file);
        out << "frequency";
...
// Multimode function for Normal softrandom - RFDynamic
template<typename MultiModeFunc>
void process_normal_softrandom_folders_multimode_RF(MultiModeFunc multi_mode_algo,
                                     const std::string& data_dir,
                                     const std::string& output_dir,
                                     int nJobsPerRound,
                                     const std::vector<int>& modes_to_run,
    ...
    for (auto& thread : folder_threads) thread.join();
    for (auto& pair : results_by_version) {
        std::string output_file = normal_softrandom_result_dir + "/normal_softrandom_result_RFDynamic_njobs" +
                                 std::to_string(nJobsPerRound) + "_" + std::to_string(pair.first) + ".csv";
        std::ofstream out(output_file);
        out << "frequency";
        for (int mode : modes_to_run) out << ",RFDynamic_njobs" << nJobsPerRound << "_mode" << mode << "_L2_norm_flow_time";
...
// Function for combination_softrandom folders (2, 3, 4 combinations)
template<typename AlgoFunc>
void process_combination_softrandom_folders(AlgoFunc algo, const std::string& algo_name,
                                           const std::string& data_dir, const std::string& output_dir) {
    std::string combination_softrandom_result_dir = output_dir + "/combination_softrandom_result";
    create_directory(combination_softrandom_result_dir);

    auto folders = list_directory(data_dir);

    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        if (basename.find("combination_softrandom_") == std::string::npos ||
            !directory_exists(folder)) {
            continue;
        }

        int base_version = extract_version_from_path(basename);
        std::cout << "Processing combination_softrandom base: " << basename
                 << " (version=" << base_version << ")" << std::endl;
        ...
        for (const auto& comb_type : combination_types) {
            ...
            std::string result_dir = combination_softrandom_result_dir + "/" + comb_type;
...
// Multimode function for combination_softrandom - Dynamic
template<typename MultiModeFunc>
void process_combination_softrandom_folders_multimode(MultiModeFunc multi_mode_algo,
                                                      const std::string& data_dir,
                                                      const std::string& output_dir,
                                                      int nJobsPerRound,
                                                      const std::vector<int>& modes_to_run,
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        if (basename.find("combination_softrandom_") == std::string::npos || !directory_exists(folder)) {
            continue;
        }

        int base_version = extract_version_from_path(basename);
        safe_cout("Processing combination_softrandom base: " + basename + "\n");
...
// Multimode function for combination_softrandom - Dynamic_BAL
template<typename MultiModeFunc>
void process_combination_softrandom_folders_multimode_DBAL(MultiModeFunc multi_mode_algo,
                                                           const std::string& data_dir,
                                                           const std::string& output_dir,
                                                           int nJobsPerRound,
                                                           const std::vector<int>& modes_to_run,
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        if (basename.find("combination_softrandom_") == std::string::npos || !directory_exists(folder)) continue;
...
// Multimode function for combination_softrandom - RFDynamic
template<typename MultiModeFunc>
void process_combination_softrandom_folders_multimode_RF(MultiModeFunc multi_mode_algo,
                                                         const std::string& data_dir,
                                                         const std::string& output_dir,
                                                         int nJobsPerRound,
                                                         const std::vector<int>& modes_to_run,
...
// Function for Bounded Pareto combination_softrandom folders (2, 3, 4 combinations)
template<typename AlgoFunc>
void process_bounded_pareto_combination_softrandom_folders(AlgoFunc algo, const std::string& algo_name,
                                       const std::string& data_dir, const std::string& output_dir) {
    std::string combination_softrandom_result_dir = output_dir + "/Bounded_Pareto_combination_softrandom_result";
    create_directory(combination_softrandom_result_dir);
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        // Look for Bounded_Pareto_combination_softrandom_ folders only (not regular Bounded_Pareto_softrandom_)
        if (basename.find("Bounded_Pareto_combination_softrandom_") == std::string::npos ||
            !directory_exists(folder)) {
            continue;
        }
...
// Function for Normal combination_softrandom folders (2, 3, 4 combinations)
template<typename AlgoFunc>
void process_normal_combination_softrandom_folders(AlgoFunc algo, const std::string& algo_name,
                                       const std::string& data_dir, const std::string& output_dir) {
    std::string combination_softrandom_result_dir = output_dir + "/normal_combination_softrandom_result";
    create_directory(combination_softrandom_result_dir);
    ...
    for (const auto& folder : folders) {
        std::string basename = folder.substr(folder.find_last_of('/') + 1);
        // Look for normal_combination_softrandom_ folders only (not regular normal_softrandom_)
        if (basename.find("normal_combination_softrandom_") == std::string::npos ||
            !directory_exists(folder)) {
            continue;
        }
...
// Multimode function for Bounded Pareto combination_softrandom - Dynamic_BAL
template<typename MultiModeFunc>
void process_bounded_pareto_combination_softrandom_folders_multimode_DBAL(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
...
// Multimode function for Normal combination_softrandom - Dynamic_BAL
template<typename MultiModeFunc>
void process_normal_combination_softrandom_folders_multimode_DBAL(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
...
// Multimode for Bounded Pareto combination_softrandom - Dynamic (regular multimode)
template<typename MultiModeFunc>
void process_bounded_pareto_combination_softrandom_folders_multimode(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
            ...
            for (const auto& freq_folder : freq_folders) {
                ...
                folder_threads.emplace_back([&, freq_folder, freq_basename]() {
                    ...
                    for (const auto& file : files) {
                        ...
                        auto jobs = read_jobs_from_csv(file);
                        if (jobs.empty()) continue;
                        auto result_pair = multi_mode_algo(jobs, nJobsPerRound, file, modes_to_run);
                        std::map<int, double> mode_results = result_pair.first;
                        std::map<int, double> max_flow_results = result_pair.second;
                        std::map<std::string, std::string> result_map;
                        result_map["frequency"] = std::to_string(frequency);
...
// Multimode for Normal combination_softrandom - Dynamic (regular multimode)
template<typename MultiModeFunc>
void process_normal_combination_softrandom_folders_multimode(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
...
// Multimode for Bounded Pareto combination_softrandom - RFDynamic (multimode_RF)
template<typename MultiModeFunc>
void process_bounded_pareto_combination_softrandom_folders_multimode_RF(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
...
// Multimode for Normal combination_softrandom - RFDynamic (multimode_RF)
template<typename MultiModeFunc>
void process_normal_combination_softrandom_folders_multimode_RF(MultiModeFunc multi_mode_algo,
                                                       const std::string& data_dir,
                                                       const std::string& output_dir,
                                                       int nJobsPerRound,
                                                       const std::vector<int>& modes_to_run,
