...
def execute_single_run(algorithm_func, job_list, checkpoint, arrival_rate, run_number, algorithm_name):
    ...
    return result
def run_algorithms_in_parallel(job_list, arrival_rate, num_runs=3):
    """
    Run all algorithms in parallel and collect results
    
    This function launches all algorithm variants (Rdynamic_sqrt_2, Rdynamic_sqrt_6, etc.)
    in parallel and waits for all to complete.
    
    Args:
        job_list: List of jobs to process
    ...
    """
    logger.info(f"Starting parallel algorithm execution for arrival_rate={arrival_rate} with {num_runs} runs each")
    
    # Use ProcessPoolExecutor for parallel algorithm execution
    results = {}
    futures = []
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=5) as executor:
        # Submit Rdynamic_sqrt_2
        for run_number in range(1, num_runs + 1):
            ...
            futures.append((future, "RDYNAMIC_SQRT_2", run_number))
        
        # Submit Rdynamic_sqrt_6
        for run_number in range(1, num_runs + 1):
            future = executor.submit(
                execute_single_run,
            ...
            futures.append((future, "RDYNAMIC_SQRT_6", run_number))
        
        # Submit Rdynamic_sqrt_8
        for run_number in range(1, num_runs + 1):
            future = executor.submit(
                execute_single_run,
            ...
            futures.append((future, "RDYNAMIC_SQRT_8", run_number))
        
        # Submit Rdynamic_sqrt_10
        for run_number in range(1, num_runs + 1):
            future = executor.submit(
                execute_single_run,
            ...
            futures.append((future, "RDYNAMIC_SQRT_10", run_number))
        ...
        for run_number in range(1, num_runs + 1):
            ...
            futures.append((future, "Dynamic", run_number))
        
        # Collect results as they complete
        for future, algorithm_name, run_number in futures:
            try:
                ...
                if result is not None:
                    ...
                    results[algorithm_name].append(result[1])  # Append L2 norm
                    logger.info(f"Completed {algorithm_name} run {run_number} with L2 norm: {result[1]}")
                else:
                    logger.error(f"{algorithm_name} run {run_number} failed or timed out")
            except concurrent.futures.TimeoutError:
                logger.error(f"Timeout for {algorithm_name} run {run_number}")
            except Exception as e:
                logger.error(f"Error in {algorithm_name} run {run_number}: {e}")
                import traceback
                logger.error(traceback.format_exc())
    ...
    # Calculate mean for each algorithm if we have results, otherwise use fallback value
    if "RDYNAMIC_SQRT_2" in results and results["RDYNAMIC_SQRT_2"]:
        processed_results["rdynamic_sqrt_2_l2"] = np.mean(results["RDYNAMIC_SQRT_2"])
    else:
        processed_results["rdynamic_sqrt_2_l2"] = 1000.0
        
    if "RDYNAMIC_SQRT_6" in results and results["RDYNAMIC_SQRT_6"]:
        processed_results["rdynamic_sqrt_6_l2"] = np.mean(results["RDYNAMIC_SQRT_6"])
    else:
        processed_results["rdynamic_sqrt_6_l2"] = 1000.0
        
    if "RDYNAMIC_SQRT_8" in results and results["RDYNAMIC_SQRT_8"]:
        processed_results["rdynamic_sqrt_8_l2"] = np.mean(results["RDYNAMIC_SQRT_8"])
    else:
        processed_results["rdynamic_sqrt_8_l2"] = 1000.0
        
    if "RDYNAMIC_SQRT_10" in results and results["RDYNAMIC_SQRT_10"]:
        processed_results["rdynamic_sqrt_10_l2"] = np.mean(results["RDYNAMIC_SQRT_10"])
    else:
        processed_results["rdynamic_sqrt_10_l2"] = 1000.0
        
    if "Dynamic" in results and results["Dynamic"]:
        processed_results["dynamic_l2"] = np.mean(results["Dynamic"])
    else:
        processed_results["dynamic_l2"] = 1000.0
    
    logger.info(f"Parallel algorithm execution completed for arrival_rate={arrival_rate}")
...
def execute_phase2(arrival_rate, bp_parameter, file_handler):
    ...
    for bp_param in bp_parameter:
        ...
        algo_results = run_algorithms_in_parallel(job_list, arrival_rate)
        
        # Extract mean L2 norms from results
        rdynamic_sqrt_2_l2 = algo_results["rdynamic_sqrt_2_l2"]
        rdynamic_sqrt_6_l2 = algo_results["rdynamic_sqrt_6_l2"]
        rdynamic_sqrt_8_l2 = algo_results["rdynamic_sqrt_8_l2"]
        rdynamic_sqrt_10_l2 = algo_results["rdynamic_sqrt_10_l2"]
        dynamic_l2 = algo_results["dynamic_l2"]
